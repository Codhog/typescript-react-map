...
export default function GeocoderControl(props: GeocoderControlProps) {
  ...
  // const {contextObj} = useContext(MapInfoContext)
  // const {setPosition} = contextObj
  const handleAdd = props.addArray
  const geocoder = useControl<MapboxGeocoder>(
    () => {
      ...
      ctrl.on('result', evt => {
        ...
        if (location) {
          console.log('handel?')
          // handleAdd({lat:location[1],lng:location[0], name:'Success'})
          setMarker(<Marker {...props.marker} longitude={location[0]} latitude={location[1]} />);
        } else {
          console.log('diu nnwei when null')
          // setMarker(null);
        }
      });
